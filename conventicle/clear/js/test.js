...
function cheat() {
    ...
    if (currentLevel == 5) {
        alert("you already cheated enough.");
    } else {
        setNewLevelToken(currentLevel + 1);
        alert("...cheater! \n passing to level " + getCurrentLevel());
    }
    location.reload();
}
...
function loadCheatButtons() {
    var value = `
			<button onclick="cheat()" id="cheat" class="button">
                <h2>CHEAT</h2>
            </button>
            <button onclick="reset()" id="reset" class="button">
                <h2>RESET</h2>
            </button>
	`;
    $("#vip").html(value);
